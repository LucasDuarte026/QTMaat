...
namespace minas_control
{
    ...
    void MinasClient::servoOn(int mode)
    {
        MinasInput input = readInputs();
    ...
    void MinasClient::setInterpolationTimePeriod(int us)
    {
        ...
        printf("Set interpolation time period %d us (%d/%d) and ret: %d\n", us, u32val, u8val, ret);
    ...
    void MinasClient::setSwitchSpeed(uint32_t val)
    {
        // 6084h, unit: pulse/s, range: 0 - 4294967295, U32
        uint32_t u32val = (uint32_t)val;
        manager_.writeSDO<uint32_t>(slave_no_, 0x6099, 0x01, u32val);
    }

    void MinasClient::setZeroSpeed(uint32_t val)
    {
        // 6084h, unit: pulse/s, range: 0 - 4294967295, U32
        uint32_t u32val = (uint32_t)val;
        manager_.writeSDO<uint32_t>(slave_no_, 0x6099, 0x02, u32val);
    }

    void MinasClient::setHomingAcceleration(uint32_t val)
    {
        // 6084h, unit: pulse/s, range: 0 - 4294967295, U32
        uint32_t u32val = (uint32_t)val;
        manager_.writeSDO<uint32_t>(slave_no_, 0x609A, 0x00, u32val);
    }

    void MinasClient::setHomingTorqueLimit(uint16_t val)
    {
        // 6084h, unit: pulse/s, range: 0 - 4294967295, U16
        uint16_t u16val = (uint16_t)val;
        manager_.writeSDO<uint16_t>(slave_no_, 0x5350, 0x00, u16val);
    }

    void MinasClient::setHomingDetectionTime(uint16_t val)
    {
        // 6084h, unit: pulse/s, range: 0 - 4294967295, U16
        uint16_t u16val = (uint16_t)val;
        manager_.writeSDO<uint16_t>(slave_no_, 0x5351, 0x00, u16val);
    }

    void MinasClient::setHomingDetectionVelocity(uint32_t val)
    {
        // 6084h, unit: pulse/s, range: 0 - 4294967295, U32
        uint32_t u32val = (uint32_t)val;
        manager_.writeSDO<uint32_t>(slave_no_, 0x5352, 0x00, u32val);
    }

    void MinasClient::setHomingMode(uint8_t val)
    {
        // 6084h, unit: pulse/s, range: 0 - 4294967295, U8
        uint8_t u8val = (uint8_t)val;
        manager_.writeSDO<uint8_t>(slave_no_, 0x6098, 0x00, u8val);
    }

    void MinasClient::setTouchProbe(uint16_t val)
    {
        // 6084h, unit: pulse/s, range: 0 - 65535, U16
        uint16_t u16val = (uint16_t)val;
        manager_.writeSDO<uint16_t>(slave_no_, 0x60B8, 0x00, u16val);
    }

    void MinasClient::setCommunicationFuntionExtendedSetup(uint16_t val)
    {
        // 6084h, unit: pulse/s, range: -32768 – 32767, U16
        uint16_t u16val = (uint16_t)val;
        manager_.writeSDO<uint16_t>(slave_no_, 0x3722, 0x00, u16val);
    }

    void MinasClient::setHomingReturnSpeedLimit(uint16_t val)
    {
        // 6084h, unit: pulse/s, range: 0 - 20000, U16
        uint16_t u16val = (uint16_t)val;
        manager_.writeSDO<uint16_t>(slave_no_, 0x3793, 0x00, u16val);
    }
